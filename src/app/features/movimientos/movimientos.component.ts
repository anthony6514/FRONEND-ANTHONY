...
export class MovimientosComponent implements OnInit {
  ...
  ngOnInit() {
    // Carga inventario (tiene stockActual real) en lugar de products (stock siempre 0)
  ...
  // Cambiar qué producto se visualiza en el kardex
  verKardex(idStr: string) {
    const id = +idStr;
    if (!id) return;
    this.productoActual.set(id);
    this.loading.set(true);
    this.ds.getKardexHttp(id).subscribe({
      next:  movs => { this.movimientos.set(movs); this.loading.set(false); },
      error: ()   => { this.loading.set(false); },
    });
  }
  ...
  filtered = computed(() => {
    ...
    const t  = this.tab();
    // "Salidas" incluye tanto SALIDA directa como AJUSTE_SALIDA
    if (t === 'entradas') list = list.filter(m =>
      m.tipoMovimiento === 'ENTRADA' || m.tipoMovimiento === 'AJUSTE_ENTRADA');
    if (t === 'salidas')  list = list.filter(m =>
  ...
  get totalEntradas()    {
    return this.movimientos().filter(m =>
      m.tipoMovimiento === 'ENTRADA' || m.tipoMovimiento === 'AJUSTE_ENTRADA').length;
  }
  get totalSalidas()     {
    return this.movimientos().filter(m =>
      m.tipoMovimiento === 'SALIDA' || m.tipoMovimiento === 'AJUSTE_SALIDA').length;
  }
  get totalAjustes()     {
    return this.movimientos().filter(m => m.tipoMovimiento.startsWith('AJUSTE')).length;
  }
  get litrosIngresados() {
    return this.movimientos()
      .filter(m => m.tipoMovimiento === 'ENTRADA' || m.tipoMovimiento === 'AJUSTE_ENTRADA')
  ...
  tipoIcon(t: string) {
    const m: Record<string, string> = {
      ENTRADA: 'arrow_downward', SALIDA: 'arrow_upward',
      AJUSTE_ENTRADA: 'add_circle', AJUSTE_SALIDA: 'remove_circle'
    };
  ...
  guardarEntrada() {
    if (!this.formEntrada.productoId || !this.formEntrada.cantidad) {
      this.errorMsg = 'Selecciona un producto e ingresa la cantidad.'; return;
    }
  ...
  guardarSalida() {
    if (!this.formSalida.productoId || !this.formSalida.cantidad) {
      this.errorMsg = 'Selecciona un producto e ingresa la cantidad.'; return;
    }
    if (this.formSalida.cantidad > this.stockDisponible) {
      this.errorMsg = `Stock insuficiente. Disponible: ${this.stockDisponible}`; return;
    }
    this.saving.set(true); this.errorMsg = '';
  ...
  guardarAjuste() {
    if (!this.formAjuste.productoId || !this.formAjuste.cantidad) {
      this.errorMsg = 'Selecciona un producto e ingresa la cantidad.'; return;
    }
    this.saving.set(true); this.errorMsg = '';
